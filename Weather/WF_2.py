...
def city_name():
    ...
    # Getting Location and timezone for the given location
    geolocator = Nominatim(user_agent="geoapiExcercises")
    location = geolocator.geocode(city)
    obj = TimezoneFinder()
    time_zone = obj.timezone_at(lng=location.longitude, lat=location.latitude)

    # Defining and Calling API
    url = "https://api.openweathermap.org/data/2.5/forecast?lat=" + str(location.latitude) + "&lon=" + str(
        location.longitude) + "&units=metric&appid=ec07412f864e92751f697214bdebd0aa"
    ...
    weather_condition = todays_data['weather'][0]['main']

    # Change background color based on weather condition
    if "clear" in weather_condition.lower():
        root['background'] = "#87CEEB"  # Light Blue for clear sky
    elif "cloud" in weather_condition.lower():
        root['background'] = "#B0C4DE"  # Light Steel Blue for cloudy weather
    elif "rain" in weather_condition.lower():
        root['background'] = "#778899"  # Light Slate Gray for rainy weather
    else:
        root['background'] = "#57adff"  # Default color
